...
config bInfo = do 
  (excode, out, err) <- readProcessWithExitCode "lhapdf-config" ["--ldflags"] ""
  liboptset' <- case excode of 
                  ExitSuccess -> do  
                    return . Just .  mkLibraryOptionSet . words $ out
                  _ -> do 
                    putStrLn $ "lhapdf-config failure." 
                    return Nothing              
  (excode2,out2,err2) <- readProcessWithExitCode "lhapdf-config" ["--incdir"] ""
...
libraryOptions :: [String] -> [LibraryOption] 
